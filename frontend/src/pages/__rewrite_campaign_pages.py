...
campaign_page_content = '''import { FormEvent, useMemo, useState } from 'react';
...
const CampaignPage = () => {
  ...
  const districts = useMemo(() => [...new Set(initialCampaigns.map((campaign) => campaign.district))], []);

  const filteredCampaigns = useMemo(
    () =>
      initialCampaigns.filter((campaign) => {
  ...
  const totals = useMemo(
    () => ({
      ...
      active: initialCampaigns.filter((campaign) => campaign.status === 'Active').length,
      planned: initialCampaigns.filter((campaign) => campaign.status === 'Planned').length,
      completed: initialCampaigns.filter((campaign) => campaign.status === 'Completed').length,
    }),
    []
  );

  const topCampaigns = useMemo(
    () => [...initialCampaigns].sort((a, b) => b.collected - a.collected).slice(0, 3),
    []
  );
...
dashboard_campaigns_content = '''import { FormEvent, useMemo, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  MapPin,
  Calendar,
...
Path('CampaignPage.tsx').write_text(campaign_page_content, encoding='utf-8')
Path('DashboardCampaigns.tsx').write_text(dashboard_campaigns_content, encoding='utf-8')
print('Wrote CampaignPage.tsx and DashboardCampaigns.tsx')
